...
class WaveletX:
    ...
    def __call__(self, x, target):
        ...
        for i in range(self.optim_steps):
            ...
            # Get grayscale waveletx for spatial reagularization
            waveletx = self.inverse_dwt((m_yl*yl_gray, [m*y for m,y in zip(m_yh, yh_gray)])).clamp(0,1)
            ...
            l1spatial = (waveletx.abs().reshape(waveletx.size(0), -1).sum(dim=-1) / (np.prod(waveletx.shape[1:]))).sum()
            l2spatial = torch.sqrt((waveletx.pow(2).reshape(waveletx.size(0), -1).sum(dim=-1) / (np.prod(waveletx.shape[1:]))).sum())
        ...
        # Final explanation
        waveletx = torch.cat(waveletx_per_rgb, dim=1).clamp(0,1)
        assert tuple(waveletx.shape)==tuple(x.shape), waveletx.shape
